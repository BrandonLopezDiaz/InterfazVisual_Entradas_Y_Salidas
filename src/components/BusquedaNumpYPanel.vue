...
<template>
    <main>
        ...
        <div class="Contenido">
            <div class="buscador">
                <label>Ingrese el numero del colaborador y panel</label>
                ...
                <select id="Panel"  >
                    ...
                    <!--Muestra los distintos paneles que hay y a su vez cuandon es seleccionado captura el valor y lo manda a otra funcion-->
                    <option v-for="tblNetwork in Panel" :value="tblNetwork.idNetwork" :key="tblNetwork.idNetwork"> {{tblNetwork.tDescNetwork}}</option>
                </select>
                <!--Cuando se le da click llama a una funcion esta funcion recibe los datos del input y select-->
                ...
                <button id="Excel" @click="tableToExcel()" class="btn btn-success" style="margin-left: 0%;">Exportar a Excel</button>
            </div>
           
                <div class="tabla" style="width: 97%;height: 1675%;">
...
export default {
    ...
    //Funcion que ejecuta automaticamente otra funcion al momento de mostrar el componente
    created: function(){
        this.ConsultarPanel();
    },
    methods: {
        ...
        async consultarpracticantes() {
            ...
            this.form.panel = document.getElementById('Panel').value;//Obtiene el valor de la id
            //Funcion de axios para el get el cual recibe 2 valores
            try
            {
